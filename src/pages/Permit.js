...
const Permit = () => {
  ...
  return (
    <CContainer fluid>
      ...
      {/* Summary Cards (Optional: connect count from API if available) */}
      <CRow className="mb-4 text-center">
        {['In Progress', 'Active', 'Rejected', 'Expired'].map((status) => (
          <CCol key={status}>
            <CCard className={`border-start-4 border-start-${statusColors[status]} bg-light`}>
        ...
        (filteredPermits.length === 0 ? (
          ...
          filteredPermits.map((item, idx) => (
            <tr key={idx}>
              ...
              <td>
                <button className="btn btn-link text-primary p-0">Detail</button>
              </td>
            </tr>
          ))
        ))
